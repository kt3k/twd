...
```
deno install --allow-read=. --allow-write=. --allow-net=deno.land,esm.sh,cdn.esm.sh -fq https://deno.land/x/twd@v0.4.2/cli.ts
```
...
```ts
import { Config } from "https://deno.land/x/twd@v0.4.2/types.ts";
...
`https://deno.land/x/twd@v0.4.2/colors.ts`.
